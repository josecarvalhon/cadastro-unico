{{
  config(
    ...
    format='parquet',
    location='s3://lakehouse/ouro/int_fornecedores_cluster.parquet'
  )
}}

{# Mesma estratégia de clientes, mas sobre fornecedores. PJs majoritariamente
   têm CNPJ válido → quase todos caem no nível doc_match. #}

with prata as (
  select * from {{ ref('int_fornecedores_padronizados') }}
),
nivel_1 as (
  select
    *,
